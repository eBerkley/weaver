...
func TestReadComponentGraph(t *testing.T) {
	for _, test := range []struct{ os, arch string }{
		...
		t.Run(fmt.Sprintf("%s/%s", test.os, test.arch), func(t *testing.T) {
			...
			pkg := "github.com/eberkley/weaver/runtime/bin/testprogram"
			main := "github.com/eberkley/weaver/Main"
	...
	for _, test := range []struct{ os, arch string }{
		...
		t.Run(fmt.Sprintf("%s/%s", test.os, test.arch), func(t *testing.T) {
			...
			pkg := func(c string) string {
				return fmt.Sprintf("github.com/eberkley/weaver/runtime/bin/testprogram/%s", c)
			}
			main := "github.com/eberkley/weaver/Main"
